...
function serverCmdUse(%client,%data)
{
   ...
   if( Game.class $= "TR2Game" )
   {
   	if( %data $= Disc )
        ...
        %client.getControlObject().use((%client.team == 1) ? TR2GoldTargetingLaser : TR2SilverTargetingLaser);
   	// etc...
   	else
...
function ShapeBase::hasAmmo( %this, %weapon )
{
   ...
   {
      if (%weapon $= TargetingLaser || %weapon $= TR2GoldTargetingLaser || %weapon $= TR2SilverTargetingLaser)
      {
         return( false );   
      }
      else 
...
function serverCmdGiveAll(%client)
{
   if($TestCheats)
   {
      ...
      %player.setInventory(TR2Mortar, 1);
      %player.setInventory(TR2Shocklance, 1);
      %player.setInventory(TR2GoldTargetingLaser, 1);
      %player.setInventory(TR2SilverTargetingLaser, 1);
      %player.setInventory(TR2DiscAmmo, 999);
      %player.setInventory(TR2GrenadeLauncherAmmo, 999);
      %player.setInventory(TR2MortarAmmo, 999);
      %player.setInventory(TR2ChaingunAmmo, 999);
      %player.setInventory(TR2Grenade, 999);
   }
}
