...
function EditorGui::init(%this)
{
   ...
   DropTypeMenu.setText(EWorldEditor.dropType);
   ...
   EditorMenuBar.addMenuItem("File", "Export World Editor Selection...", 0);
   EditorMenuBar.addMenuItem("File", "Import World Editor Selection...", 0);
   EditorMenuBar.addMenuItem("File", "-", 0);
   ...
   EditorMenuBar.addMenu("AI", 8);
   EditorMenuBar.addMenuItem("AI", "Render Indoor Graphs", 3);
   EditorMenuBar.addMenuItem("AI", "Render Outdoor Graph", 4);
   EditorMenuBar.addMenuItem("AI", "Render Jet Edges", 4);
   EditorMenuBar.addMenuItem("AI", "-", 0);
   EditorMenuBar.addMenuItem("AI", "Edit AI for Team 1", 4);
   EditorMenuBar.addMenuItem("AI", "Edit AI for Team 2", 7);
   EditorMenuBar.addMenuItem("AI", "-", 0);
   EditorMenuBar.addMenuItem("AI", "Build Objectives", 5);
   EditorMenuBar.addMenuItem("AI", "Build Bot Nav Graph...", 5);
   EditorMenuBar.addMenuItem("AI", "Build Spawn Graph...", 5);
   EditorMenuBar.addMenuItem("AI", "Build LOS Crossrefs...", 5);
   EditorMenuBar.addMenuItem("AI", "-", 0);
   EditorMenuBar.addMenuItem("AI", "Add Defend Location Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Repair Equipment Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Deploy Outdoor Turret Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Deploy Indoor Turret Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Attack Target Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Touch FlipFlop Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Mortar Target Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add Deploy Inventory Objective", 5);
   EditorMenuBar.addMenuItem("AI", "Add New Objective Set", 6);
   ...
   EditorMenuBar.addMenuItem("Window", "AI Editor", 10);
   ...
   EditorMenuBar.setMenuItemChecked("AI", "Render Indoor Graphs", $pref::NavGraph::drawIndoor);
   EditorMenuBar.setMenuItemChecked("AI", "Render Outdoor Graph", $pref::NavGraph::drawOutdoor);
   EditorMenuBar.setMenuItemChecked("AI", "Render Jet Edges", $pref::NavGraph::drawJetEdges);
   $AIEditor::inspectTeam = 1;
   EditorMenuBar.setMenuItemChecked("AI", "Edit AI for Team 1", $AIEditor::inspectTeam == 1);
   EditorMenuBar.setMenuItemChecked("AI", "Edit AI for Team 2", $AIEditor::inspectTeam == 2);

   ETerrainEditor.getPrefs();
   EditorMenuBar.onActionMenuItemSelect(0, "Paint");
   ...
   if(isObject(MissionGroup) && isObject(MissionCleanup))
   {
      MissionGroup.add(MissionCleanup);
      MissionCleanup.setPersistent(false);
   }
}

function EditorNewMission()
{
   if(ETerrainEditor.isMissionDirty || ETerrainEditor.isDirty || EWorldEditor.isDirty || EWorldEditor.isGraphDirty)
   {
...
//exec("temp\\day.cs");
//exec("temp\\night.cs");
//MissionGroup.add(day);
//MissionGroup.add(night);
...
function EditorDoSaveAs(%missionName)
{
   ...
   EWorldEditor.isGraphDirty = true;
   %saveMissionFile = $CurrentMission;
   %saveTerrName = Terrain.terrainFile;

   $CurrentMission = fileBase(%missionName);
   Terrain.terrainFile = fileBase(%missionName) @ ".ter";

   if(!EditorSaveMission())
   {
      $CurrentMission = %saveMissionFile;
...
{
   // gotta strip off the missions path
   %file = fileBase(%file);
   loadMission( %file, $CurrentMissionType, true ) ;
   // and re-init the EditorGui
   EditorGui.init();
...
function EditorMenuBar::onMenuSelect(%this, %menuId, %menu)
{
   if(%menu $= "File")
   {
      ...
      EditorMenuBar.setMenuItemEnable("File", "Save Mission...", EWorldEditor.isGraphDirty || ETerrainEditor.isDirty || ETerrainEditor.isMissionDirty || EWorldEditor.isDirty);
      EditorMenuBar.setMenuItemEnable("File", "Export World Editor Selection...", EWorldEditor.isVisible() && EWorldEditor.getSelectionSize());
      EditorMenuBar.setMenuItemEnable("File", "Import World Editor Selection...", EWorldEditor.isVisible());
   }
   else if(%menu $= "Edit")
   {
   ...
   else if(%menu $= "World")
   {
      ...
      EditorMenuBar.setMenuItemEnable("World", "Delete Selection", (!EditorTree.isSelectionEmpty() || %selSize > 0) && %lockCount == 0);
   }
}
...
function EditorMenuBar::onMenuItemSelect(%this, %menuId, %menu, %itemId, %item)
{
   switch$(%menu)
   {
      ...
      case "AI":
         %this.onAIMenuItemSelect(%itemId, %item);
   }
}
...
function EditorMenuBar::onFileMenuItemSelect(%this, %itemId, %item)
{
   switch$(%item)
   {
      ...
      case "Save Mission As...":
         EditorSaveMissionAs();
      case "Export World Editor Selection...":
         EWorldEditor.export();
      case "Import World Editor Selection...":
         EWorldEditor.import();
...
function WorldEditor::doImport(%this, %file)
{
   ...
   while(%grp.getCount())
   {
      EWorldEditor.selectObject(%grp.getObject(0));
      %tempInstant.add(%grp.getObject(0));
   }
   EWorldEditor.dropSelection();

   EditorTree.fillSelectionFromWorldEditor(%this);
   $InstantGroup = %tempInstant;
   %grp.delete();
}
...
function EditorGui::setAIEditTeam(%this, %team)
{
   $AIEditor::inspectTeam = %team;
   EditorMenuBar.setMenuItemChecked("AI", "Edit AI for Team 1", $AIEditor::inspectTeam == 1);
   EditorMenuBar.setMenuItemChecked("AI", "Edit AI for Team 2", $AIEditor::inspectTeam == 2);
   EditorTree.open("MissionGroup/Teams/team" @ %team @ "/AIObjectives");
   $InstantGroup = nameToId("MissionGroup/Teams/team" @ %team @ "/AIObjectives");
}

function EditorGui::makeUnbridgedGraph()
{
   GraphCurrentOperation.setValue("Generating interior nodes...");
   Canvas.repaint();   
   navGraph::generateInterior();
   EditorGui.dataProgress();
   
   GraphCurrentOperation.setValue("Remaking graph...");
   Canvas.repaint();   
   EditorGui.remakeTheGraph();
   EditorGui.dataProgress();
   EWorldEditor.isGraphDirty = true;
}
...
function EditorGui::dataProgress(%this)
{
...
function EditorGui::remakeTheGraph(%this)
{
...
function EditorGui::createLOSXref(%this)
{
   EditorGui.AIworking(true);
      ...
      Canvas.repaint();   
   EWorldEditor.isGraphDirty = true;
   EditorGui.AIworking(false);
}

function EditorGui::makeJettableGraph(%this, %NAVorSPAWN)
{
   %this.AIworking(true);
...
function EditorMenuBar::onAIMenuItemSelect(%this, %itemId, %item)
{
   switch$(%item)
   {
      case "Build Objectives":
         AICreateObjectives();
      case "Build Bot Nav Graph...":
         EditorGui.makeJettableGraph(Nav);
      case "Build Spawn Graph...":
         EditorGui.makeJettableGraph(Spawn);
      case "Build LOS Crossrefs...":
         EditorGui.createLOSXref();
      case "Render Indoor Graphs":
         $pref::NavGraph::drawIndoor = !$pref::NavGraph::drawIndoor;
         EditorMenuBar.setMenuItemChecked("AI", "Render Indoor Graphs", $pref::NavGraph::drawIndoor);
      case "Render Outdoor Graph":
         $pref::NavGraph::drawOutdoor = !$pref::NavGraph::drawOutdoor;
         EditorMenuBar.setMenuItemChecked("AI", "Render Outdoor Graph", $pref::NavGraph::drawOutdoor);
      case "Render Jet Edges":
         $pref::NavGraph::drawJetEdges = !$pref::NavGraph::drawJetEdges;
         EditorMenuBar.setMenuItemChecked("AI", "Render Jet Edges", $pref::NavGraph::drawJetEdges);
      case "Edit AI for Team 1":
         EditorGui.setAIEditTeam(1);
      case "Edit AI for Team 2":
         EditorGui.setAIEditTeam(2);
      case "Add New Objective Set":
         %set = new SimGroup("ObjectiveSet");
         $InstantGroup.add(%set);
      default:
         EditorGui.addNewObjective(getWord(%item, 1) SPC getWord(%item, 2));
   }
}

function EditorGui::addNewObjective(%this, %type)
{
   ...
   EWorldEditor.clearSelection();
   EWorldEditor.selectObject(%objective);
   EWorldEditor.dropSelection();
...
function EditorGui::setWorldEditorVisible(%this)
{
   ...
   EditorMenuBar.setMenuVisible("AI", false);
   EWorldEditor.schedule(0, makeFirstResponder, true);
}
...
function EditorGui::setTerrainEditorVisible(%this)
{
   ...
   EditorMenuBar.setMenuVisible("AI", false);
   ETerrainEditor.schedule(0, makeFirstResponder, true);
...
function EditorGui::setEditor(%this, %editor)
{
   ...
   if(%this.currentEditor $= "AI Editor" && %this.lastInstantGroup !$= "")
   {
      %grp = nameToID(%this.lastInstantGroup);
      if(%grp != -1)
         $InstantGroup = %grp;
   }
   %this.currentEditor = %editor;
   $AIEdit = false;
   EWorldEditor.renderNav = false;
   // add objclasses which we do not want to collide with
   
   EWorldEditor.clearIgnoreList();
   EWorldEditor.ignoreObjClass(TerrainBlock, Sky, AIObjective);
   EWorldEditor.toggleIgnoreList = false;
   ...
   switch$(%editor)
   {
      ...
      case "Mission Area Editor":
         ...
         %this.setWorldEditorVisible();
      case "AI Editor":
         if(%this.currentEditor !$= "AI Editor")
            %this.lastInstantGroup = $InstantGroup;
         EWFrame.setVisible(true);
         EWMissionArea.setVisible(false);
         EWCreatorPane.setVisible(false);
         EWInspectorPane.setVisible(true);
         %this.setWorldEditorVisible(true);
         EditorMenuBar.setMenuVisible("AI", true);
         EWorldEditor.clearIgnoreList();
         EWorldEditor.ignoreObjClass(AIObjective);
         EWorldEditor.toggleIgnoreList = true;
         EWorldEditor.renderNav = true;
         EditorGui.setAIEditTeam($AIEditor::inspectTeam);
...
function EditorGui::getHelpPage(%this)
{
   switch$(%this.currentEditor)
   {
      case "World Editor" or "World Editor Inspector" or "World Editor Creator":
         return "3. World Editor";
      case "Mission Area Editor":
         return "4. Mission Area Editor";
      case "Terrain Editor":
         return "5. Terrain Editor";
      case "Terrain Terraform Editor":
         return "6. Terrain Terraform Editor";
      case "Terrain Texture Editor":
         return "7. Terrain Texture Editor";
      case "Terrain Texture Painter":
         return "8. Terrain Texture Painter";
      default:
         return "2. Mission Editor Overview";
   }
}
...
function ETerrainEditor::changeMaterial(%this, %matIndex)
{
   ...
   getLoadFilename("textures/terrain/*.png\ttextures/terrain/*.jpg", EPainterChangeMat, true);
}
...
function EPainter::setup(%this)
{
   ...
   for(%i = 0; %i < 6; %i++)
   {
      ...
      %this.mat[%i] = %mat;
      ("ETerrainMaterialText" @ %i).setText(%mat);
      ("ETerrainMaterialBitmap" @ %i).setBitmap("terrain/" @ %mat);
      ("ETerrainMaterialChange" @ %i).setActive(true);
      ("ETerrainMaterialPaint" @ %i).setActive(%mat !$= "");
      if(%mat $= "")
      {
         ("ETerrainMaterialChange" @ %i).setValue("Add...");
         if(%valid)
            %valid = false;
         else
         ...
         ("ETerrainMaterialChange" @ %i).setValue("Change...");
   }
...
function WorldEditor::onSelectionChanged(%this)
{
   EditorTree.fillSelectionFromWorldEditor(%this);   
}
...
function Texture::addMaterialTexture()
{
   ...
   getLoadFilename("textures/terrain/*.png\ttextures/terrain/*.jpg", addLoadedMaterial);
}
...
{        
   %name = fileBase(%name) @ fileExt(%name);
   terraformer.saveGreyscale($TerraformerSaveRegister, "terrains/heightfield/" @ %name);
}
...
function Heightfield::save()
{
   ...
   echo("Heightfield Script:" @ %script);
...
//--------------------------------------
function Heightfield::loadFromScript(%script)
{
   echo(%script);
